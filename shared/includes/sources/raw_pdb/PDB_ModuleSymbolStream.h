...
namespace PDB
{
	...
	public:
		...
		// Returns raw stream size. The first 4 bytes are the module stream signature.
		PDB_NO_DISCARD inline size_t GetSize(void) const PDB_NO_EXCEPT
		{
			return m_stream.GetSize();
		}

		// Returns a record at a module-symbol-stream-relative offset.
		// This is needed to resolve S_PROCREF/S_LPROCREF records from the global stream.
		PDB_NO_DISCARD inline const CodeView::DBI::Record* GetRecordAtOffset(size_t offset) const PDB_NO_EXCEPT
		{
			if ((offset + sizeof(CodeView::DBI::RecordHeader)) > m_stream.GetSize())
			{
				return nullptr;
			}
			...
			if ((offset + sizeof(CodeView::DBI::RecordHeader) + recordSize) > m_stream.GetSize())
			{
				return nullptr;
			}
